...
export default function AdminPage() {
  ...
  useEffect(() => {
      ...
      const channels = supabase
          ...
          .on(
              'postgres_changes',
              { event: '*', schema: 'public', table: 'check_ins' }, // Might fetch slightly too often if unrelated check_ins change, but fetchLineupData filters
              (payload) => {
                  console.log('Check-in change received:', payload);
                  fetchLineupData(activeDateId);
              }
          )
          .on(
  ...
  // fetchLineupData is memoized by useCallback implicitly via useState/setState, but adding it is safer if it were defined outside. Let's assume it's stable for now.
  }, [activeDateId, message]); // Re-run when activeDateId changes OR if message changes (e.g., from error to non-error state)
  ...
  async function fetchLineupData(dateId: string | null) {
      ...
      // Optional: Indicate background refresh slightly?
      // console.log('Fetching lineup data for date:', dateId);
  ...
  async function handleCheckIn(signupId: string) {
    ...
    try {
      const existingMember = lineupMembers.find(m => m.id === signupId && m.type === 'signup');
      if (!existingMember) throw new Error('Signup not found');
      ...
      // Subscription will trigger refresh, but provide immediate feedback
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
          ...
          <form onSubmit={handleAddWalkIn} className="space-y-4">
            ...
            <button
              ...
              className="w-full py-2 px-4 bg-secondary text-secondary-foreground rounded-lg font-medium hover:bg-secondary/80 focus:outline-none focus:ring-2 focus:ring-secondary/20 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
            >
              ...
              >
                ...
                <div className="text-sm text-right w-full sm:w-auto flex justify-end items-center gap-3 pl-9 sm:pl-0">
                    {member.isCheckedIn ? (
                        ...
                        // Only show check-in button for signups
                        member.type === 'signup' && (
                            <button
                                onClick={() => handleCheckIn(member.id)}
                                disabled={checkingInId === member.id || checkingInId !== null} // Disable if this one or any other is being processed
                                className="py-1 px-3 bg-primary text-white rounded font-medium text-sm hover:bg-primary-light focus:outline-none focus:ring-2 focus:ring-primary/20 focus:ring-offset-1 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 whitespace-nowrap"
                            >
